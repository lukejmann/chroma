...
const ErrorList = {
  ...
  [ControlsErrors.ALREADY_REGISTERED_TYPE]: (type: string) => [
    `Type ${type} has already been registered. You can't register a component with the same type.`,
  ],
  [ControlsErrors.CLIPBOARD_ERROR]: (value: unknown) => [
    `Error copying the value`,
    value,
  ],
  [ControlsErrors.THEME_ERROR]: (category: string, key: string) => [
    `Error accessing the theme \`${category}.${key}\` value.`,
  ],
  [ControlsErrors.PATH_DOESNT_EXIST]: (path: string) => [
  ...
  [ControlsErrors.INPUT_TYPE_OVERRIDE]: (
    path: string,
    type: string,
    wrongType: string
  ) => [
  ...
  [ControlsErrors.EMPTY_KEY]: () => [
    "Keys can not be empty, if you want to hide a label use whitespace.",
  ],
};

function _log<T extends ControlsErrors>(
  fn: "log" | "warn",
  errorType: T,
  ...args: Parameters<typeof ErrorList[T]>
) {
  //@ts-expect-error
