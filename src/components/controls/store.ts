...
export const Store = function (this: StoreType) {
    ...
    store.setState((s) => {
      ...
      Object.entries(newData).forEach(([path, newInputData]) => {
        ...
        if (!!input) {
          ...
          if (type !== input.type) {
            warn(ControlsErrors.INPUT_TYPE_OVERRIDE, type);
          } else {
  ...
  const _getDataFromSchema = (
    ...
    Object.entries(schema).forEach(([key, rawInput]: [string, any]) => {
      ...
      if (key === "") return warn(ControlsErrors.EMPTY_KEY);
      ...
      if (rawInput.type === SpecialInputs.FOLDER) {
        ...
        if (normalizedInput) {
          ...
          warn(ControlsErrors.UNKNOWN_INPUT, newPath, rawInput);
        }
      }
    });
...
export const controlsStore = new Store();
