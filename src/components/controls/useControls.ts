...
import { useRenderRoot } from "./components/Controls";
import type {
  FolderSettings,
  Schema,
  SchemaToValues,
...
>(
  ...
  // GlobalPanel means that no store was provided, therefore we're using the controlsStore
  const isGlobalPanel = !hookSettings?.store;

  useRenderRoot(isGlobalPanel);
  const [store] = useState(() => hookSettings?.store || controlsStore);
